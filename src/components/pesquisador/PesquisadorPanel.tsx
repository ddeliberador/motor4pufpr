...
const PesquisadorPanel = () => {
  ...
  const loadIcts = () => {
    ...
    // Via edge function: a chave institucional do backend fica no servidor.
    supabase.functions
      .invoke("ict-search", {
        body: {
          query: data.query,
      ...
      .then(({ data: result, error }) => {
        if (result && !result.error && !error) setIcts(result);
        else setIctsError(result?.error || error?.message || "Falha na consulta.");
      })
