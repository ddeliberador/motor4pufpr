...
const PesquisadorPanel = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    else { await search(searchQuery, "pesquisador"); }
  };

  const handleCnaeConfirm = async (selected: CnaeCode[]) => { setSelectedCnaes(selected); setShowCnaeModal(false); await search(pendingSearchQuery, "pesquisador"); };
  ...
  const institutionRanking = knowledge ? Object.entries(knowledge.institutions || {}).sort((a, b) => (b[1] as number) - (a[1] as number)).slice(0, 10) : [];
  ...
  const institutionsWithContracts = policy ? new Set(policy.contracts.map(c => c.organ?.toLowerCase().slice(0, 15)).filter(Boolean)) : new Set<string>();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-16">
        ...
        <div className="sticky top-16 z-30 bg-background/95 backdrop-blur-sm border-b border-border">
          <div className="max-w-6xl mx-auto px-4 py-3">
            <div className="flex items-center gap-3">
              <Link to="/" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors flex-shrink-0">
              ...
              <form onSubmit={handleSearch} className="flex-1 flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 rounded-xl border border-border bg-card pl-9 pr-4 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                    autoFocus
                  />
                </div>
                <Button type="submit" disabled={isLoading || !searchQuery.trim()} size="sm" className={`rounded-xl bg-gradient-to-r ${config.color} text-white px-4 gap-1.5`}>
              ...
              {data && (
                <span className="text-[9px] px-2 py-1 bg-primary/10 text-primary rounded-full border border-primary/20 flex-shrink-0 hidden sm:inline">
                  {data.meta.source_count} fontes
                </span>
              )}
            </div>
            {data && (
              <p className="text-[10px] text-muted-foreground mt-1 ml-[4.5rem] truncate">
          ...
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Conhecimento · Tecnologia · Política · Internacional</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Results */}
        {data && knowledge && technology && policy && intl && (
