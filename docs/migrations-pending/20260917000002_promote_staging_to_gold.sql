...
--
-- Justificativa de plpgsql vs Edge Function:
--   Edge Function orquestrando REST não tem transação.
--   Falha de rede entre chamadas deixa Gold apagado e Staging não promovido.
--   plpgsql garante atomicidade dentro de uma única transação Postgres.
--
-- Depende de: 20260917000001_create_staging_locations.sql
--
-- Verificação (idempotência):
--   SELECT * FROM promote_staging_to_gold('openalex', 70, 'test');
--   -- primeira chamada: promoted > 0
--   SELECT * FROM promote_staging_to_gold('openalex', 70, 'test');
--   -- segunda chamada: promoted = 0
...
COMMENT ON FUNCTION public.promote_staging_to_gold IS
  'ADR-0001 ARQ-02. Promove staging_locations → research_locations em transação única. Idempotente. Segunda execução retorna promoted=0.';
