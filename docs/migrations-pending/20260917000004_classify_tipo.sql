...
-- Função classify_tipo(): replica a lógica de src/components/mapa/dataLake.ts no banco
-- Alimenta staging_locations.canonical_type durante a ingestão
-- Quando dataLake.ts for removido (ARQ-07), esta função é a única fonte de verdade
--
-- Verificação:
--   SELECT classify_tipo('Universidade Federal do Paraná', 'ICT', '{}');
--   -- deve retornar 'Universidade'
--   SELECT classify_tipo('Startup XYZ', 'Startup', '{}');
--   -- deve retornar 'Startup'
...
AS $$
  SELECT CASE
    ...
    WHEN lower(COALESCE(p_tipo_raw,'')) LIKE '%ict%'
      OR lower(COALESCE(p_tipo_raw,'')) LIKE '%instituiç%'
      THEN 'ICT'
...
COMMENT ON FUNCTION public.classify_tipo IS
  'ADR-0001 ARQ-05. Replica classificação de tipo de dataLake.ts no banco. Fonte de verdade após ARQ-07.';
