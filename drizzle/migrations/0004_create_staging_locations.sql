...
DROP POLICY IF EXISTS "staging_admin_all" ON staging_locations;
CREATE POLICY "staging_admin_all"
  ON staging_locations FOR ALL
  USING (public.is_admin());

DROP POLICY IF EXISTS "staging_auth_select" ON staging_locations;
...
COMMENT ON TABLE staging_locations IS
  'Camada Staging do ADR-0001. Recebe dado bruto das APIs, calcula quality_score e canonical_type. Só promove para research_locations (Gold) via promote_staging_to_gold() quando score >= 70. anon nunca tem acesso.';