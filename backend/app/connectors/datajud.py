"""
MOTOR 4P UFPR - DataJud Connector
Conector para dados judiciais do CNJ

Fonte: https://datajud.cnj.jus.br/ via dados.gov.br
"""
import logging
...
class DataJudConnector(BaseConnector):
    """
    Conector para DataJud — processos judiciais, estatísticas
    de desempenho dos tribunais, mandados de prisão.
    """
    ...
    async def search_datasets(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        logger.info(f"DataJud search: {query}")
        try:
            data = await self.get(
                f"{self.base_url}/package_search",
                params={
                    "q": f"{query} justiça judicial tribunal CNJ",
                    "rows": str(limit),
                },
                use_cache=True,
            )
